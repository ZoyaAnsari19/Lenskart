...
const orderResolvers = {
  Order: {
    ...
    status: (order) => order.orderStatus || 'pending',
    products: (order) => {
      // Map items to products format, handling missing fields
      return (order.items || []).map(item => ({
        id: item.product?._id?.toString() || item.product?.toString() || 'unknown',
  ...
  Query: {
    orders: async (parent, args, { user }) => {
      // Return empty array if no user or no orders
      if (!user) return [];
    ...
    order: async (parent, { id }, { user }) => {
      ...
      // If no user (public track order), allow access but return limited data
